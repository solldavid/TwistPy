...
class PolarizationModel:
    ...
    Attributes:
        ...
        scaling_velocity: scaling velocity (m/s) to make translations dimensionless, Default: 1 (m/s)
    ...
    def __init__(self, scaling_velocity: float = 1, free_surface: bool = True, wave_type: str = None,
                 vp: np.ndarray = None,
    ...
    def polarization_vector(self):
        ...
        if self.wave_type == 'P':
            ...
            if self.free_surface:
                ...
                theta_p[index] = np.arcsin(np.sin(theta_rad[index]) * self.vp[index] / self.vs[index]).astype('complex')
            ...
            phi_rad = np.radians(self.phi)
            self.xi = np.radians(self.xi)
            v_x = -1j * 1. / self.scaling_velocity * np.sin(self.xi) * np.cos(phi_rad)
            v_y = 1j * 1. / self.scaling_velocity * np.sin(self.xi) * np.sin(phi_rad)
            v_z = 1. / self.scaling_velocity * np.cos(self.xi)
