...
class DispersionAnalysis:
    ...
    def __init__(
        ...
        fmax: float = 20.0,
        octaves: float = 0.5,
        window: dict = {"number_of_periods": 1, "overlap": 0.0},
        ...
        for i in range(len(f_lower)):
            if verbose:
                print(
                    f"Estimating surface wave parameters at frequency f = {f_center_tot[i]: 0.2f} Hz. "
                    f"Frequency step {i + 1} out of {len(f_lower)}."
                )
            window_f = {
                "window_length_seconds": 1
                / f_center_tot[i]
                * window["number_of_periods"],
            ...
            parameters_f = {
                "xi": analysis.xi,
                "phi_r": analysis.phi_r,
                "phi_l": analysis.phi_l,
                "c_l": analysis.c_l,
                "c_r": analysis.c_r,
                "wave_types": analysis.classification["0"],
                "dop": analysis.dop,
            }
    ...
    def plot(
        self,
        nbins: int = 100,
        velocity_range: Tuple[float, float] = (0, 2500),
        ...
        for i, f in enumerate(self.f):
            ...
            counts, values_elli = np.histogram(
                elli, bins=nbins, range=[-90, 90], density=True
            )
    ...
    def plot_baz(self, freq: float = None, nbins: int = 90, show: bool = True) -> None:
        ...
        if freq is not None:
            ...
            height_phi_r, _ = np.histogram(
                phi_r, nbins, range=(0, 2 * np.pi), density=True
            )
            height_phi_r /= height_phi_r.max() * 1.2
            height_phi_l, _ = np.histogram(
                phi_l, nbins, range=(0, 2 * np.pi), density=True
            )
