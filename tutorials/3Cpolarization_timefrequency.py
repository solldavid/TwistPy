...
# frequency-direction the window extends over 2 Hz.
...
analysis.plot_polarization_analysis(major_semi_axis=True, clip=0.05, show=False)
...
########################################################################################################################
# Once the polarization attributes are computed, you can access them as attributes. The available attributes are: 'dop'
# (degree of polarization), 'elli' (Ellipticity), 'inc1' (Inclination of the major semi-axis of the polarization
# ellipse), 'inc2' (Inclination of the minor semi-axis), 'azi1' (Azimuth of the major semi-axis), and 'azi2' (Azimuth of
# the minor semi-axis). So, if you want to access the ellipticity, you would do:
...
data_filtered = analysis.filter(elli=[0.0, 0.2], dop=[0.7, 1], inc1=[0, 40])
...
data_filtered_rayleigh = analysis.filter(elli=[0.7, 1.0], dop=[0.7, 1])

########################################################################################################################
# To filter out parts of the signal that exhibit particle motion along a specific direction (e.g., an azimuth of 60
# degrees), we could use:

data_filtered_50degrees_azimuth = analysis.filter(azi1=[55, 65], dop=[0.7, 1])
